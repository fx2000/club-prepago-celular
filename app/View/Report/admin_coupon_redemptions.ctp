<?php
/**
 * Redemption Coupons Report View
 *
...
<div class="row-fluid ">
	<div class="box span12">
		<div class="box-header well" data-original-title>
			<h2><i class="icon-user"></i><?php echo __(' Redemption Coupons'); ?></h2>
		</div>
			...
			<table class="table table-striped table-bordered bootstrap-datatable Reptrxdatatable">
				<thead>
					 <tr>
						...
						<th class="hidden-phone"><?php echo __('Coupon ID'); ?></th>
						<th><?php echo __('Amount'); ?></th>
						<th><?php echo __('Purchase Date'); ?></th>
						<th class="hidden-phone"><?php echo __('Redeem Date'); ?></th>
				...
				<tbody>
					...
					<tr>
						<td class="hidden-phone">
							<?php
								echo $this->Html->link(
									$val['CouponRedemption']['reference_no'],
									array(
										'controller' => 'Recharge',
										'action'     => 'view_status',
										base64_encode($val['CouponRedemption']['reference_no'])
									)
								);
							?>
						</td>
						<td class="hidden-phone">
							<?php
								//if ($val['CouponRedemption']['user_type'] == 1) {
									$userId = $val['CouponRedemption']['user_id'];
									if ($userId != 0) {
										echo $this->Html->link(
											$val['User']['name'],
											array(
						...
						<td class="hidden-phone">
							<?php
								...
								/*  $userId =*/ echo $val['CouponRedemption']['coupon_id'];
								//	if ($userId != 0) {
								//		echo $this->Html->link(
								//			$val['User']['name'],
								//			array(
						...
						<td><?php echo 'B/. ' . $val['Coupon']['amount']; ?></td>
						<td><?php echo $val['CouponRedemption']['purchase_date']; ?></td>
						<td><?php echo $val['CouponRedemption']['redeem_date']; ?></td>
						<td class="center">
							<?php
								if ($val['CouponRedemption']['status'] == 0) {
									echo __("<span class='label label-warning'>Pending</span>");
								} else if ($val['CouponRedemption']['status'] == 1) {
									echo __("<span class='label label-success'>Applyed</span>");
								} else if ($val['CouponRedemption']['status'] == 2) {
									echo __("<span class='label label-warning'>Replaced</span>");
								} else {
									echo $val['CouponRedemption']['status'];
								}
							?>
						</td>
					</tr>
