 ...
 */
include "Request.GetCoupons.php";
...
class RestResponse {
    ...
    //$logger->info("============================================================");
    //$logger->info("Received GetCoupons request:", $CLIENT_DATA_ARY);
    ...
    // If parameter check fails, send an error message
		if (in_array("F",$returnArray)) {
			$logger->error("GetCoupons request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('501'));
      ...
      // Check User ID
        ...
        $userId = $CLIENT_DATA_ARY['UserId'];
          ...
          //$logger->error("GetCoupons request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('541'));
					return $this->generateJSONError('541');

      	} else if (!preg_match("/^[0-9\+]+$/i", stripslashes($userId))) {

          //$logger->error("GetCoupons request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('542'));
        ...
        //$logger->error("GetCoupons request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('540'));
      ...
      // Check Device ID
			if (in_array("DeviceId", $client_key_array)) {
				...
				if (strlen($deviceId) == 0) {
					...
					//$logger->error("GetCoupons request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('503'));
        ...
        //$logger->error("GetCoupons request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('502'));
				return $this->generateJSONError('502');

      }

      // If all fields are validated correctly, call the Get Coupons API
			$REQ_SUCCESS = new RequestGetCouponsAPI();
      ...
      // Check that User ID is valid
        ...
        //$logger->error("GetCoupons request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('543'));
        return $this->generateJSONError('543');
			...
			if ($deviceIdValid == 0) {
				...
				//$logger->error("GetCoupons request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('544'));
        return $this->generateJSONError('544');

      }

      // Get coupons
		  $coupons = $REQ_SUCCESS->getCoupons($CLIENT_DATA_ARY);
          ...
          $arr['Data'] = $coupons;
          $result['Response'] = $arr;
          //$logger->info("GetCoupons request successful for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . json_encode($result));
          return json_encode($result);

        } else {

          //$logger->error("GetCoupons request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('586'));
          return $this->generateJSONError('586');

        }

      }

	 	} // End function generateResponse
