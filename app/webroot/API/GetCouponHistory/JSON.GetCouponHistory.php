 ...
 */
include "Request.GetCouponHistory.php";
...
class RestResponse {
	...
	function generateResponse($CLIENT_DATA_ARY) {
		...
		if (in_array("F", $returnArray)) {
			...
			// $logger->error("GetCouponHistory request failed for UserId " . $userId . " " . $this->generateJSONError('501'));
			...
			if (in_array("UserId", $client_key_array)) {
				...
				if (strlen($userId) == 0) {
					...
					// $logger->error("GetCouponHistory request failed for UserId " . $userId . " " . $this->generateJSONError('541'));
					return $this->generateJSONError('541');

				} else if (!preg_match("/^[0-9\+]+$/i", stripslashes($userId))) {

					// $logger->error("GetCouponHistory request failed for UserId " . $userId . " " . $this->generateJSONError('542'));
				...
				// $logger->error("GetCouponHistory request failed for UserId " . $userId . " " . $this->generateJSONError('540'));
			...
			if (in_array("DeviceId", $client_key_array)) {
				...
				if (strlen($deviceId) == 0) {
					...
					// $logger->error("GetCouponHistory request failed for UserId " . $userId . " " . $this->generateJSONError('503'));
				...
				// $logger->error("GetCouponHistory request failed for UserId " . $userId . " " . $this->generateJSONError('502'));
				return $this->generateJSONError('502');

			}

			// If all fields are validated correctly, call the Get Coupon History API
			$REQ_SUCCESS = new RequestGetCouponHistoryAPI();
			...
			if ($userIdValid == 0) {
				...
				// $logger->error("GetCouponHistory request failed for UserId " . $userId . " " . $this->generateJSONError('543'));
			...
			if ($deviceIdValid == 0) {
				...
				// $logger->error("GetCouponHistory request failed for UserId " . $userId . " " . $this->generateJSONError('544'));
				return $this->generateJSONError('544');

			}

			// Request Coupon history
			$couponHistory = $REQ_SUCCESS->getCouponHistory($CLIENT_DATA_ARY);
			...
			if ($couponHistory > 0) {
				...
				$arr['Code'] = $status;
				$arr['Message'] = $output;
				$arr['Data'] = $couponHistory;
				$result['Response'] = $arr;
				// $logger->info("GetCouponHistory request successful for UserId " . $userId . " " . json_encode($result));
