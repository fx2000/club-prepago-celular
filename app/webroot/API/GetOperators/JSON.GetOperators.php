 ...
 */
include "Request.GetOperators.php";
...
class RestResponse {
	...
	function generateResponse($CLIENT_DATA_ARY) {
		...
		$logger->info("============================================================");
		$logger->info("Received GetOperators request:", $CLIENT_DATA_ARY);
		...
		if (in_array("F",$returnArray)) {
			...
			if (in_array("DeviceId", $client_key_array)) {
				...
				if (strlen($deviceId) == 0) {
					$logger->error("GetOperators request failed " . $this->generateJSONError('503'));
					return $this->generateJSONError('503');
				}
			} else {
				$logger->error("GetOperators request failed " . $this->generateJSONError('502'));
			...
			if (in_array("PlatformId", $client_key_array)) {
				...
				if (strlen($platformId) == 0) {
					$logger->error("GetOperators request failed " . $this->generateJSONError('526'));
					return $this->generateJSONError('526');
				}
			} else {
				$logger->error("GetOperators request failed " . $this->generateJSONError('527'));
				return $this->generateJSONError('527');
			}

			// If all fields are validated correctly, call the Get Operators API
			$REQ_SUCCESS = new RequestGetOperatorsAPI();
			...
			$operators = $REQ_SUCCESS->getOperators($CLIENT_DATA_ARY);
			...
			if ($operators > 0) {
				...
				$arr['Data'] = $operators;
				$result['Response'] = $arr;
				$logger->info("GetOperators request successful " . json_encode($result));
				return json_encode($result);

			// Otherwise, return appropriate error code
			} else {
				$logger->error("GetOperators request failed " . $this->generateJSONError('554'));
