 ...
 */
include "Request.GetPrepaidBalance.php";
...
class RestResponse {
	...
	function generateResponse($CLIENT_DATA_ARY) {
		...
		$logger->info("============================================================");
		$logger->info("Received GetPrepaidBalance request:", $CLIENT_DATA_ARY);
		...
		if (in_array("F", $returnArray)) {
			$logger->error("GetPrepaidBalance request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('501'));
			...
			if (in_array("UserId", $client_key_array)) {
				...
				if (strlen($userId) == 0) {
					$logger->error("GetPrepaidBalance request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('541'));
					return $this->generateJSONError('541');
				} else if (!preg_match("/^[0-9\+]+$/i", stripslashes($userId))) {
					$logger->error("GetPrepaidBalance request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('542'));
					return $this->generateJSONError('542');
				}
			} else {
				$logger->error("GetPrepaidBalance request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('540'));
			...
			if (in_array("DeviceId", $client_key_array)) {
				...
				if (strlen($deviceId) == 0) {
					$logger->error("GetPrepaidBalance request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('503'));
					return $this->generateJSONError('503');
				}
			} else {
				$logger->error("GetPrepaidBalance request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('502'));
			...
			if (in_array("PlatformId", $client_key_array)) {
				...
				if (strlen($platformId) == 0) {
					$logger->error("GetPrepaidBalance request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('526'));
			...
			// If all fields are validated correctly, call the Get Prepaid Balance API
			$REQ_SUCCESS = new RequestGetPrepaidBalanceAPI();

			// Check that Platform is valid
			$platformValid = $REQ_SUCCESS->checkPlatform($platformId);

			if ($platformValid == 0) {
				$logger->error("GetPrepaidBalance request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('527'));
				return $this->generateJSONError('527');
			}
			...
			if ($deviceIdValid == 0) {
				$logger->error("GetPrepaidBalance request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('544'));
				return $this->generateJSONError('544');
			}

			// Request prepaid balance
			$balance = $REQ_SUCCESS->getPrepaidBalance($CLIENT_DATA_ARY);
			...
			if ($balance > 0) {
				...
				$arr['Data'] = $balance;
				$result['Response'] = $arr;
				$logger->info("GetPrepaidBalance request successful for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . json_encode($result));
				return json_encode($result);
			}

			// Otherwise, return appropriate error code
			else {
				$logger->error("GetPrepaidBalance request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('553'));
				return $this->generateJSONError('553');
			}
		}
	}
