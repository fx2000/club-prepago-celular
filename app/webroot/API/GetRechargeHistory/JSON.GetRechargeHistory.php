 ...
 */
include "Request.GetRechargeHistory.php";
...
class RestResponse {
	...
	function generateResponse($CLIENT_DATA_ARY) {
		...
		$logger->info("============================================================");
		$logger->info("Received GetRechargeHistory request:", $CLIENT_DATA_ARY);
		...
		if (in_array("F", $returnArray)) {
			$logger->error("GetRechargeHistory request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('501'));
			...
			if (in_array("UserId", $client_key_array)) {
				...
				if (strlen($userId) == 0) {
					$logger->error("GetRechargeHistory request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('541'));
					return $this->generateJSONError('541');
				} else if (!preg_match("/^[0-9\+]+$/i", stripslashes($userId))) {
					$logger->error("GetRechargeHistory request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('542'));
					return $this->generateJSONError('542');
				}
			} else {
				$logger->error("GetRechargeHistory request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('540'));
			...
			if (in_array("DeviceId", $client_key_array)) {
				...
				if (strlen($deviceId) == 0) {
					$logger->error("GetRechargeHistory request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('503'));
					return $this->generateJSONError('503');
				}
			} else {
				$logger->error("GetRechargeHistory request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('502'));
			...
			if (in_array("PlatformId", $client_key_array)) {
				...
				if (strlen($platformId) == 0) {
					$logger->error("GetRechargeHistory request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('526'));
					return $this->generateJSONError('526');
				}
			} else {
				$logger->error("GetRechargeHistory request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('527'));
				return $this->generateJSONError('527');
			}

			// If all fields are validated correctly, call the Get Recharge History API
			$REQ_SUCCESS = new RequestGetHistoryAPI();
			...
			if ($userIdValid == 0) {
				$logger->error("GetRechargeHistory request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('543'));
			...
			if ($deviceIdValid == 0) {
				$logger->error("GetRechargeHistory request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('544'));
				return $this->generateJSONError('544');
			}

			// Request recharge history
			$rechargeHistory = $REQ_SUCCESS->getRechargeHistory($CLIENT_DATA_ARY);

			// If everything went well, return result
			if ($rechargeHistory > 0) {
				$status = '533';
				...
				$arr['Code'] = $status;
				$arr['Message'] = $output;
				$arr['Data'] = $rechargeHistory;
				$result['Response'] = $arr;
				$logger->info("GetRechargeHistory request successful for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . json_encode($result));
				return json_encode($result);
			}

			// Otherwise, return appropriate error code
			else {
				$logger->error("GetRechargeHistory request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('587'));
