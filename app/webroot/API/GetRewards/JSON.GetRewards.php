...
class RestResponse {
	...
	function generateResponse($CLIENT_DATA_ARY) {
		...
		$logger->info("============================================================");
		$logger->info("Received GetRewards request:", $CLIENT_DATA_ARY);
		...
		if (in_array("F",$returnArray)) {
			$logger->error("GetRewards request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('501'));
			...
			if (in_array("UserId", $client_key_array)) {
				...
				if (strlen($userId) == 0) {
					...
					$logger->error("GetRewards request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('542'));
					return $this->generateJSONError('542');
				}
			} else {
				$logger->error("GetRewards request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('540'));
			...
			if (in_array("DeviceId", $client_key_array)) {
				...
				$logger->error("GetRewards request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('502'));
			...
			if (in_array("PlatformId", $client_key_array)) {
				...
				$logger->error("GetRewards request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('527'));
				return $this->generateJSONError('527');
			}

			// If all fields are validated correctly, call the Get Rewards API
			$REQ_SUCCESS = new RequestGetRewardsAPI();
			...
			if ($userIdValid == 0) {
				$logger->error("GetRewards request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('543'));
			...
			if ($deviceIdValid == 0) {
				$logger->error("GetRewards request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('544'));
				return $this->generateJSONError('544');
			}

			// Get rewards
			$rewards = $REQ_SUCCESS->getRewards($CLIENT_DATA_ARY);
			...
			if ($rewards != 0) {
				...
				$arr['Data'] = $rewards;
				$result['Response'] = $arr;
				$logger->info("GetRewards request successful for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . json_encode($result));
				return json_encode($result);
			} else {
				$logger->error("GetRewards request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('586'));
