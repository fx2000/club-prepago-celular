...
class RequestRechargePointsAPI extends Dbconn {
			...
			if ($status == '00') {
				...
				if ($data['PlatformId'] == 1) {
					$updUserPoints = $this->fireQuery(
						"UPDATE users
							SET points = points - " . $data['Points'] . " WHERE id = ". $data['UserId']
					);
				}
	...
	function lowInventoryEmail($operator) {
		...
		// Check Staff table for receipient email address
		$selStaff =
			"SELECT *
				FROM staff";
		$resStaff = $this->fireQuery($selStaff);
		$arrStaff = $this->fetchAssoc($resStaff);
		...
		if ($arrMinLimit['balance'] <= $arrMinLimit['minimum_limit']) {
			...
			$mail->addAddress($arrStaff['email'], $arrStaff['name']);
			$mail->WordWrap = 50;
			$mail->isHTML(true);
			$mail->CharSet = "UTF-8";
	...
	/**
	 * Check a user's available points
	 */
