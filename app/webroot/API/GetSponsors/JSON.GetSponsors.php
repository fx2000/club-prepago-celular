 ...
 */
include "Request.GetSponsors.php";
...
class RestResponse {
	...
	function generateResponse($CLIENT_DATA_ARY) {
		...
		$logger->info("============================================================");
		$logger->info("Received GetSponsors request:", $CLIENT_DATA_ARY);
		...
		if (in_array("F",$returnArray)) {
			$logger->error("GetSponsors request failed " . $this->generateJSONError('501'));
			...
			if (in_array("DeviceId", $client_key_array)) {
				...
				if (strlen($deviceId) == 0) {
					$logger->error("GetSponsors request failed " . $this->generateJSONError('503'));
					return $this->generateJSONError('503');
				}
			} else {
				$logger->error("GetSponsors request failed " . $this->generateJSONError('502'));
			...
			if (in_array("PlatformId", $client_key_array)) {
				...
				if (strlen($platformId) == 0) {
					$logger->error("GetSponsors request failed " . $this->generateJSONError('526'));
					return $this->generateJSONError('526');
				}
			} else {
				$logger->error("GetSponsors request failed " . $this->generateJSONError('527'));
				return $this->generateJSONError('527');
			}

			// If all fields are validated correctly, call the Get Sponsor API
			$REQ_SUCCESS = new RequestGetSponsorsAPI();
			...
			$sponsors = $REQ_SUCCESS->getSponsors($CLIENT_DATA_ARY);
			...
			if ($sponsors > 0) {
				...
				$arr['Data'] = $sponsors;
				$result['Response'] = $arr;
				$logger->info("GetSponsors request successful " . json_encode($result));
				return json_encode($result);
			} else {
				$logger->error("GetSponsors request failed " . $this->generateJSONError('703'));
				return $this->generateJSONError('703');
			}
		}
	}
