 ...
 */
include "Request.GetFavorites.php";
...
class RestResponse {
	...
	function generateResponse($CLIENT_DATA_ARY) {
		...
		$logger->info("============================================================");
		$logger->info("Received GetFavorites request:", $CLIENT_DATA_ARY);
		...
		if (in_array("F", $returnArray)) {
			$logger->error("GetFavorites request failed for UserId " . $userId . " " . $this->generateJSONError('501'));
			...
			if (in_array("UserId", $client_key_array)) {
				...
				if (strlen($userId) == 0) {
					$logger->error("GetFavorites request failed for UserId " . $userId . " " . $this->generateJSONError('541'));
					return $this->generateJSONError('541');
				} else if (!preg_match("/^[0-9\+]+$/i", stripslashes($userId))) {
					$logger->error("GetFavorites request failed for UserId " . $userId . " " . $this->generateJSONError('542'));
					return $this->generateJSONError('542');
				}
			} else {
				$logger->error("GetFavorites request failed for UserId " . $userId . " " . $this->generateJSONError('540'));
			...
			if (in_array("DeviceId", $client_key_array)) {
				...
				if (strlen($deviceId) == 0) {
					$logger->error("GetFavorites request failed for UserId " . $userId . " " . $this->generateJSONError('503'));
					return $this->generateJSONError('503');
				}
			} else {
				$logger->error("GetFavorites request failed for UserId " . $userId . " " . $this->generateJSONError('502'));
			...
			if (in_array("PlatformId", $client_key_array)) {
				...
				if (strlen($platformId) == 0) {
					$logger->error("GetFavorites request failed for UserId " . $userId . " " . $this->generateJSONError('526'));
					return $this->generateJSONError('526');
				}
			} else {
				$logger->error("GetFavorites request failed for UserId " . $userId . " " . $this->generateJSONError('527'));
				return $this->generateJSONError('527');
			}

			// If all fields are validated correctly, call the Get Favorites API
			$REQ_SUCCESS = new RequestGetFavoritesAPI();
			...
			if ($userIdValid == 0) {
				$logger->error("GetFavorites request failed for UserId " . $userId . " " . $this->generateJSONError('543'));
			...
			if ($deviceIdValid == 0) {
				$logger->error("GetFavorites request failed for UserId " . $userId . " " . $this->generateJSONError('544'));
				return $this->generateJSONError('544');
			}

			// Request list of favorites
			$Contacts = $REQ_SUCCESS->getFavorites($CLIENT_DATA_ARY);
			...
			if ($Contacts != 0) {
				...
				$arr['Data'] = $Contacts;
				$result['Response'] = $arr;
				$logger->info("GetFavorites request successful for UserId " . $userId . " " . json_encode($result));
				return json_encode($result);

			// Otherwise, return appropriate error code
			} else {
				$logger->info("GetFavorites request failed for UserId " . $userId . " " . $this->generateJSONError('547'));
				return $this->generateJSONError('547');
			}
		}
	}
