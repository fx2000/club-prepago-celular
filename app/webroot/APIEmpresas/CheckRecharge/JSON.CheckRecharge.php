 ...
 * @package       API.CheckRecharge
 * @since         Club Prepago Celular(tm) v 1.0.0
 */
include "../../API/GetRechargeHistory/Request.GetRechargeHistory.php";
...
class RestResponse {
	...
	function generateResponse($CLIENT_DATA_ARY) {
		...
		$logger->info("============================================================");
		$logger->info("Received CheckRecharge request:", $CLIENT_DATA_ARY);
		...
		if (in_array("F", $returnArray)) {
			...
			if (in_array("ReferenceNo", $client_key_array)) {
				...
				if (strlen($referenceNo) == 0) {
					$logger->error("ReferenceNo parameter cannot be empty " . $this->generateJSONError('603'));
					return $this->generateJSONError('603');
				}
			} else {
				$logger->error("ReferenceNo parameter not found " . $this->generateJSONError('604'));
			...
			// Send results back
				...
				// Request recharge info
				$rechargeInfo = $REQ_SUCCESS->checkTransacionId($CLIENT_DATA_ARY);
				...
				if ($rechargeInfo > 0) {
					...
					$arr['Code'] = $status;
					$arr['Message'] = $output;
					$arr['Data'] = $rechargeInfo;
					$result['Response'] = $arr;
					$logger->info("CheckRecharge request successful for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . json_encode($result));
					return json_encode($result);
				}

				// Otherwise, return appropriate error code
				else {
					$logger->error("CheckRecharge request failed for UserId " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('587'));
