 ...
 * @package       APIEmpresas.GetBanks
 * @since         Club Prepago Celular(tm) v 1.0.0
 */
include "../../API/GetBanks/Request.GetBanks.php";
		...
		if (in_array("F",$returnArray)) {
			...
			// Send results back
				...
				if ($banks != 0) {
					...
					$arr['Data'] = $banks;
					$result['Response'] = $arr;
					$logger->info("GetBanks request successful for User " . $CLIENT_DATA_ARY['UserId'] . " " . json_encode($result));
					return json_encode($result);

				// Otherwise, return appropriate error code
				} else {
					$logger->error("GetBanks request failed for User " . $CLIENT_DATA_ARY['UserId'] . " " . $this->generateJSONError('594'));
