 ...
 * @package       APIEmpresas.ChangePassword
 * @since         Club Prepago Celular(tm) v 1.0.0
 */
 include "../../API/ChangePassword/Request.ChangePassword.php";
      ...
      // Check User ID
			...
			if (in_array("NewPassword", $client_key_array)) {
				...
				if (strlen($newPassword) == 0) {
					$logger->error("NewPassword request failed for UserId " . $CLIENT_DATA_ARY['Email'] . " " . $this->generateJSONError('515'));
					return $this->generateJSONError('515');
				} else if (strlen($newPassword) < 6) {
					$logger->error("NewPassword request failed for UserId " . $CLIENT_DATA_ARY['Email'] . " " . $this->generateJSONError('516'));
					return $this->generateJSONError('516');
				}
			} else {
				$logger->error("NewPassword request failed for UserId " . $CLIENT_DATA_ARY['Email'] . " " . $this->generateJSONError('514'));
